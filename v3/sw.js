...
self.addEventListener('fetch', event => {
  ...
  // This worker is served from /v3/sw.js, so its scope is /v3/ and the root
  // app never reaches it. Belt and braces: anything outside the archive is
  // not ours to answer.
  if (!url.pathname.startsWith('/v3/') && !/\/(data\.json|config\.js|supabase-auth\.js|pm-ui\.js)$/.test(url.pathname)
      && !url.pathname.startsWith('/images/') && !url.pathname.startsWith('/assets/')) return;
